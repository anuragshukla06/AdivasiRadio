...
public class WebCrawler {
    ...
    private HashSet<String> getCrawledURLs() {
        HashSet<String> dbCrawledUrls = new HashSet<>();
        ...
        if (mCursor != null && mCursor.getCount() > 0) {
            ...
            for (int i = 0; i < count; i++) {
                dbCrawledUrls.add(mCursor.getString(url_columnIndex));
                mCursor.moveToNext();
            }
        }
        return dbCrawledUrls;
    }
    ...
    private class CrawlerRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (!TextUtils.isEmpty(pageContent.toString())) {
                ...
                Elements links = doc.select("div.report h3 a[href]");
                for (Element link : links) {
                    String extractedLink = link.attr("href");
                    if (!TextUtils.isEmpty(extractedLink)) {
                        synchronized (lock) {
                            String full_url = MediaActivity.CGNET_ROOT_URL + "/" + extractedLink;
                            if (!crawledURL.contains(full_url))
                                uncrawledURL.add(full_url);
                        }
                    }
                }
                // End JSoup
